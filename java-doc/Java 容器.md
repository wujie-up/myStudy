...
- TreeSet：底层是一个TreeMap, 基于红黑树实现，支持有序性操作，例如根据一个范围查找元素的操作。但是查找效率不如 HashSet，HashSet 查找的时间复杂度为 O(1)，TreeSet 则为 O(logN)。

- HashSet：底层是一个HashMap, 基于哈希表实现，支持快速查找，但不支持有序性操作。并且失去了元素的插入顺序信息，也就是说使用 Iterator 遍历 HashSet 得到的结果是不确定的。

- LinkedHashSet：底层是一个LinkedHashMap, 具有 HashSet 的查找效率，并且内部使用双向链表维护元素的插入顺序。
...
<div align="center"> <img height=400" width="600" src="https://cs-notes-1256109796.cos.ap-guangzhou.myqcloud.com/image-20191208225301973.png"/> </div><br>
...
需要调用 System.arraycopy() 将 index+1 后面的元素都往前移动（如果不是删除最后一个数），该操作的时间复杂度为 O(N)，可以看到 ArrayList 删除元素的代价是非常高的。
...
- HashMap 可以插入键为 null 的 Entry。HashTable的key和value都不能为null。
...
JDK 1.8 使用了 CAS 操作来支持更高的并发度，如果数组索引上没有则通过CAS修改，如果数组索引上有了，则通过synchronized锁住索引位置的头节点，然后修改
...
重写了HashMap的newNode方法来保证有序性
```java
Node<K,V> newNode(int hash, K key, V value, Node<K,V> e) {
    LinkedHashMap.Entry<K,V> p =
        new LinkedHashMap.Entry<K,V>(hash, key, value, e);
    linkNodeLast(p); // 通过这里维护链表
    return p;
}
```
